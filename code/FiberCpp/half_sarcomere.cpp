    ...
    double delta_hsl, double sim_mode, double set_pCa)
{
    ...
    // Update the step_counter %pickup
...
int half_sarcomere::return_m_transition(double time_step, int m_counter, int cb_counter)
{
    ...
    // max_m_transitions is how many ways you can get out of current state
        ...
        if (new_state > 0)
        {
            ...
            if (p_trans->transition_type == 'a')
            {
                ...
                for (int bs_counter = 0; bs_counter < m_attachment_span; bs_counter++)
                {
                    ...
                    prob_index = (t_counter * m_attachment_span) + bs_counter; //figure out what this is
                    gsl_vector_set(transition_probs, prob_index, prob); //setting prob_index of trans prob to prob that was calculated
