	...
	int mybpc_state, int mybpc_iso, short int active_neigh, half_sarcomere* p_hs)
{
	...
	// Sarcomere Model Transitions
	if (!strcmp(rate_type, "k12"))
	{
		double T = 300;
		...
		// rate = 800 * exp(-0.25*B*kxb*x_ext*x_ext);
		rate = 800 * exp(-0.25*B*kxb*x_nm*x_nm);
		// std::cout << "k12: " << rate << std::endl;
		// std::cout << "k12 x_ext: " << x_ext << std::endl;
		// std::cout << "k12 x: " << x << " nm" << std::endl;
	}

	if (!strcmp(rate_type, "k21"))
	{
		double T = 300;
		double kB = 1.380649e-23;
		double kxb = 3e-3;
		double B = 1/(kB*T);
		double dG_bind = -4*kB*T;
		double kbind = 800;
		double k12;
		double K12;

		// Calculate
		// k12 = kbind * exp(-0.25*B*kxb*x_ext*x_ext);
		// K12 = exp(-B*(0.5*kxb*x_ext*x_ext + dG_bind));
		k12 = kbind * exp(-0.25*B*kxb*x_nm*x_nm);
		K12 = exp(-B*(0.5*kxb*x_nm*x_nm + dG_bind));
		rate = k12 / K12;
		// std::cout << "k21:" << rate << std::endl;
		// std::cout << "k21 x_ext: " << x_ext << std::endl;
		// std::cout << "k21 x: " << x << " nm" << std::endl;
	}

	if (!strcmp(rate_type, "k23"))
	{
		double k23_cap = 300;

		rate = k23_cap;
		// std::cout << "k23: " << rate << std::endl;
		// std::cout << "k23 x_ext: " << x_ext << std::endl;
		// std::cout << "k23 x: " << x << " nm" << std::endl;
	}

	if (!strcmp(rate_type, "k32"))
	{
		//had to add in model.json
		double k23_cap = 300;
		double T = 300;
		double kB = 1.380649e-23;
		double kxb = 3e-3;
		double dG_stroke = -4.5*kB*T;
		double dG_C = -14*kB*T;
		double d_ps = 10e-9;
		double B = 1/(kB*T);
		double conc_P = 3e-3/1;
		double ADP = 0.03e-3;
		double ADPPi = ADP*conc_P;
		double k23;
		double K23;

		// K23 = exp(-B * (dG_stroke - dG_C + (0.5*kxb*(2*x_ext*d_ps+(d_ps*d_ps))) + 1/B*log(ADPPi)));
		K23 = exp(-B * (dG_stroke - dG_C + (0.5*kxb*(2*x_nm*d_ps+(d_ps*d_ps))) + 1/B*log(ADPPi)));
		k23 = k23_cap;
		rate = k23 / K23;
		// std::cout << "k32: " << rate << std::endl;
		// std::cout << "k32 x_ext: " << x_ext << std::endl;
		// std::cout << "k32 x: " << x << " nm" << std::endl;
	}

	if (!strcmp(rate_type, "k31"))
	{
		// double kATP0 = 1e-2;
		double T = 300;
		double kB = 1.380649e-23;
		...
		// K31 = exp(B * (dG_bind + dG_stroke + dG_hyd - dG_C + 0.5 * kxb * pow(x_ext + d_ps, 2) + 1/B*log(ATP)));
		K31 = exp(B * (dG_bind + dG_stroke + dG_hyd - dG_C + 0.5 * kxb * pow(x_nm + d_ps, 2) + 1/B*log(ATP)));
		// k13 = kATP0;
		// rate = K31 / k13;
		// rate = K31 * k13; correct ?
		rate = K31;
		// std::cout << "k31: " << rate << std::endl;
		// std::cout << "k31 x_ext: " << x_ext << std::endl;
		// std::cout << "k31 x: " << x  << " nm" << std::endl;		
	}
